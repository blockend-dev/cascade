...
status) that `LineageRegistry` already owns; splitting it out would mean
...
`finalizeEdge`) are functions on `LineageRegistry` itself, operating
directly on `LineageEdge.status`, `.stake`, and `.challengeDeadline`. No
separate contract.

## Consequences
- One fewer contract to deploy, audit, and keep in sync.
- `LineageRegistry` is a larger contract than a minimal registry would be.
