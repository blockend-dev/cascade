...
| Cascade `CascadeRegistry` / `ExecutionRegistry` / `AttributionSettlement` | Arithmetic and state-transition correctness (open source, audited) | Standard smart-contract risk — a bug could misroute funds, mitigated by audit and the invariants in `docs/security-invariants.md`, not by this document |
