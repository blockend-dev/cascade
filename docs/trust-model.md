...
| Cascade `CascadeRegistry` / `ExecutionRegistry` / `AttributionSettlement` / `TrainingProvenanceRegistry` | Arithmetic and state-transition correctness (open source, audited) | Standard smart-contract risk — a bug could misroute funds, mitigated by audit and the invariants in `docs/security-invariants.md`, not by this document |
| `TrainingProvenanceRegistry` records | A registered provider non-repudiably signed the declared claim fields; the claim's base/result commitments match what's independently registered in `CascadeRegistry` | **Not trusted for**: that the declared training actually produced the declared output — that binding is unconfirmed in 0G's public implementation. A false pairing with a `CascadeRegistry` edge is a false Level 2 claim, challengeable exactly as Level 3 is (ADR 0010) |
| Cascade `ExecutionRegistry`'s `CascadeWrapper` provider-mode flag | **Still owner-gated, still not on-chain-cryptographically enforced** — `setProviderMode` is unchanged from Phase 3 (ADR 0011: full on-chain TDX/NVIDIA quote verification remains out of scope). **What Phase 7 changed**: the owner's decision is now based on a documented, independently-repeatable off-chain procedure (`wrapper/MEASUREMENT.md`) — reproduce the wrapper's build, verify a provider's live attestation against it using the same manual tools 0G's own docs point to (`dstack-verifier`, `sigstore`). | A malicious or mistaken owner attestation still makes a `Declared`-strength usage proof read as `CryptographicallyBound` — the flag is checkable and falsifiable by anyone re-running `MEASUREMENT.md`'s procedure, not self-verifying. Still must never be described as "Level 1, cryptographically proven" without this caveat. |
| `wrapper/` (the Cascade-authored serving container) | Deployability confirmed from primary sources (ADR 0011) — not yet deployed and independently attestation-verified against live 0G infrastructure, which this repository has no access to | If compromised or substituted post-verification, `MEASUREMENT.md`'s procedure would only catch it on the next re-check — continuous re-attestation is disclosed future work, not built (`docs/threat-model.md` #20) |
